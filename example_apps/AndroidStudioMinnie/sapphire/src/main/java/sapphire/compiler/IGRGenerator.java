...
public class IGRGenerator {
	...
	public static String removeExtension(String s) {
	    ...
	    // Remove the extension.CannyEdgeDynamic
	...
	public static void generateStubs(String srcFolder, String packageName, String destFolder) {
		...
		for (File file : fList){
			if (file.isFile() && file.getName().endsWith("ImageGradient_Reflection.class")) {
				try {
					Class<?> c = Class.forName(IGRGenerator.removeExtension(packageName + "." + file.getName()));
					...
					if (SapphirePolicy.class.isAssignableFrom(c)) {
						...
						IGRGenerator.generateStub("policy", sapphireServerPolicyClass, destFolder);
						IGRGenerator.generateStub("policy", sapphireGroupPolicyClass, destFolder);
					}
					else
						if (SapphireObject.class.isAssignableFrom(c))
							IGRGenerator.generateStub("app", c, destFolder);
				} catch (Exception e) {
					e.printStackTrace();
				}
