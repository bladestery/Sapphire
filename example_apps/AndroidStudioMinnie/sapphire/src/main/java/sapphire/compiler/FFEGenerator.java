...
public class FFEGenerator {
	...
	public static void generateStubs(String srcFolder, String packageName, String destFolder) {
		...
		for (File file : fList){
			if (file.isFile() && file.getName().endsWith("FactoryFeatureExtractor.class")) {
				//System.out.println("processing: " + file);
				try {
					Class<?> c = Class.forName(FFEGenerator.removeExtension(packageName + "." + file.getName()));
					...
					if (SapphirePolicy.class.isAssignableFrom(c)) {
						...
						FFEGenerator.generateStub("policy", sapphireServerPolicyClass, destFolder);
						FFEGenerator.generateStub("policy", sapphireGroupPolicyClass, destFolder);
					}
					else
						if (SapphireObject.class.isAssignableFrom(c))
							FFEGenerator.generateStub("app", c, destFolder);
				} catch (Exception e) {
					e.printStackTrace();
				}
