...
public class GrayS16pairGenerator {
	...
	public static void generateStubs(String srcFolder, String packageName, String destFolder) {
		...
		for (File file : fList){
			if (file.isFile() && file.getName().endsWith("GrayS16pair.class")) {
				try {
					Class<?> c = Class.forName(GrayS16pairGenerator.removeExtension(packageName + "." + file.getName()));
					...
					if (SapphirePolicy.class.isAssignableFrom(c)) {
						...
						GrayS16pairGenerator.generateStub("policy", sapphireServerPolicyClass, destFolder);
						GrayS16pairGenerator.generateStub("policy", sapphireGroupPolicyClass, destFolder);
					}
					else
						if (SapphireObject.class.isAssignableFrom(c))
							GrayS16pairGenerator.generateStub("app", c, destFolder);
				} catch (Exception e) {
					e.printStackTrace();
				}
