...
public class GIMOGenerator {
	...
	public static void generateStubs(String srcFolder, String packageName, String destFolder) {
		...
		for (File file : fList){
			if (file.isFile() && file.getName().endsWith("GImageMiscOps.class")) {
				try {
					Class<?> c = Class.forName(GIMOGenerator.removeExtension(packageName + "." + file.getName()));
					...
					if (SapphirePolicy.class.isAssignableFrom(c)) {
						...
						GIMOGenerator.generateStub("policy", sapphireServerPolicyClass, destFolder);
						GIMOGenerator.generateStub("policy", sapphireGroupPolicyClass, destFolder);
					}
					else
						if (SapphireObject.class.isAssignableFrom(c))
							GIMOGenerator.generateStub("app", c, destFolder);
				} catch (Exception e) {
					e.printStackTrace();
				}
