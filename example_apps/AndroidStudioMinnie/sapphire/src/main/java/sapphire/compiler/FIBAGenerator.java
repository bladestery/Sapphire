...
public class FIBAGenerator {
	...
	public static void generateStubs(String srcFolder, String packageName, String destFolder) {
		...
		for (File file : fList){
			if (file.isFile() && file.getName().endsWith("FactoryImageBorderAlgs.class")) {
				//System.out.println("processing: " + file);
				try {
					Class<?> c = Class.forName(FIBAGenerator.removeExtension(packageName + "." + file.getName()));
					...
					if (SapphirePolicy.class.isAssignableFrom(c)) {
						...
						FIBAGenerator.generateStub("policy", sapphireServerPolicyClass, destFolder);
						FIBAGenerator.generateStub("policy", sapphireGroupPolicyClass, destFolder);
					}
					else
						if (SapphireObject.class.isAssignableFrom(c))
							FIBAGenerator.generateStub("app", c, destFolder);
				} catch (Exception e) {
					e.printStackTrace();
				}
