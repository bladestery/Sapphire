...
public class CEDGenerator {
	...
	public static void generateStubs(String srcFolder, String packageName, String destFolder) {
		...
		for (File file : fList){
			if (file.isFile() && file.getName().endsWith("CannyEdgeDynamic.class")) {
				try {
					Class<?> c = Class.forName(CEDGenerator.removeExtension(packageName + "." + file.getName()));
					...
					if (SapphirePolicy.class.isAssignableFrom(c)) {
						...
						CEDGenerator.generateStub("policy", sapphireServerPolicyClass, destFolder);
						CEDGenerator.generateStub("policy", sapphireGroupPolicyClass, destFolder);
					}
					else
						if (SapphireObject.class.isAssignableFrom(c))
							CEDGenerator.generateStub("app", c, destFolder);
				} catch (Exception e) {
					e.printStackTrace();
				}
