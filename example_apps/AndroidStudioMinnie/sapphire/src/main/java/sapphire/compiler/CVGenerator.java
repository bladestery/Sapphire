			...
			if (file.isFile() && file.getName().endsWith(".class")) {
				...
				try {
					...
					if (SapphirePolicy.class.isAssignableFrom(c)) {
						...
						CVGenerator.generateStub("policy", sapphireServerPolicyClass, destFolder);
						CVGenerator.generateStub("policy", sapphireGroupPolicyClass, destFolder);
					}
					else
						if (SapphireObject.class.isAssignableFrom(c))
							CVGenerator.generateStub("app", c, destFolder);
				} catch (Exception e) {
					e.printStackTrace();
				}
