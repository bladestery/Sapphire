...
public class BEDPGenerator {
	...
	public static void generateStubs(String srcFolder, String packageName, String destFolder) {
		...
		for (File file : fList){
			if (file.isFile() && file.getName().endsWith("BinaryEllipseDetectorPixel.class")) {
				try {
					Class<?> c = Class.forName(BEDPGenerator.removeExtension(packageName + "." + file.getName()));
					...
					if (SapphirePolicy.class.isAssignableFrom(c)) {
						...
						BEDPGenerator.generateStub("policy", sapphireServerPolicyClass, destFolder);
						BEDPGenerator.generateStub("policy", sapphireGroupPolicyClass, destFolder);
					}
					else
						if (SapphireObject.class.isAssignableFrom(c))
							BEDPGenerator.generateStub("app", c, destFolder);
				} catch (Exception e) {
					e.printStackTrace();
				}
