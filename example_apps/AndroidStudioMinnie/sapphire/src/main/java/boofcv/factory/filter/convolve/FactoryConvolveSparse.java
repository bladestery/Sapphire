...
public class FactoryConvolveSparse {
	...
	ImageConvolveSparse<T,K> convolve2D(Class<T> imageType, K kernel, GeneralizedImageOps GIO) {
	...
	ImageConvolveSparse<T,K> vertical1D(Class<T> imageType, K kernel, GeneralizedImageOps GIO) {
		if( GIO.isFloatingPoint(imageType)) {
	...
	public static <T extends ImageGray, K extends Kernel1D>
	ImageConvolveSparse<T,K> horizontal1D(Class<T> imageType, K kernel, GeneralizedImageOps GIO) {
		if( GIO.isFloatingPoint(imageType)) {
